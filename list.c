...
void write(BITMAPFILEHEADER file_header, RGBTRIPLE pixel, FILE* input, FILE* output, BITMAPINFOHEADER info_header, RGBTRIPLE** img, int Width, int Height) {
	...
	//ðàñ÷åò íîâîãî çíà÷åíèÿ padding
	int padding = 0;
	if ((Height * 3) % 4)
...
void read(const char* file_in, const char* file_out) {
	...
	/*÷òåíèå BMPFILEHEADER è BMPINFOHEADER*/
	...
	/*äëèíà ñòðîêè äîëæíû áûòü êðàòíà 4, èíà÷å ôàéë áóäåò ïîâðåæäåí.
	Áóäåì ïðîâåðÿòü ýòî óñëîâèå.
	padding - êîëè÷åñòâî áàéò, íåäîñòàþùèõ äî òîãî ÷òîáû êîë-âî áàéòû áûëî êðàòíî 4*/
	unsigned char padding = 0;
	if ((Width * 3) % 4)
		padding = 4 - (Width * 3) % 4;

	/*âûäåëÿåì ïàìÿòü äëÿ ìàññèâà äàííûõ î ïèêñåëÿõ*/
	...
	for (int i = 0; i < Height; i++) {
		...
		fseek(input, padding, SEEK_CUR); // ïðîïóñê "âûðàâíèâàþùèõ" áàéòîâ
	}
	fclose(input);
